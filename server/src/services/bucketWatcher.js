...
// Folder-to-doctype mappings now live in the folder_mappings database table,
// editable from the Settings tab — nothing here is hardcoded anymore.
...
function startBucketPolling() {
  ...
  const run = async () => {
    let mappings;
    try {
      const { rows } = await pool.query('SELECT prefix, doc_type FROM folder_mappings');
      mappings = rows;
    } catch (err) {
      console.error('Could not load folder mappings:', err.message);
      return;
    }
    mappings.forEach(({ prefix, doc_type }) => {
      pollPrefix(client, prefix, doc_type).catch((err) => console.error(`Poll of "${prefix}" failed:`, err.message));
    });
  };
